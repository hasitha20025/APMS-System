...
if (isset($_GET['user_id']) && isset($_GET['batch_id']) && isset($_GET['semester_id'])) {
   ...
   // echo "Batch ID: $batch_id <br>";
    ...
    if ($res_subjects) {
        ...
        echo "<h3 class='text-dark mb-0' id='Semester-Result'>6 Semester&nbsp;Result&nbsp;</h3>"; //edit hreeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee//eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
        echo "<button class='btn btn-primary bg-info border rounded-pill' type='button' onclick=\"window.location.href='../lecturesResult(edit)/Result(edit)6.php?user_id=$user_id&batch_id=$batch_id&semester_id=6'\"><i class='far fa-edit'></i>Edit</button>";
        echo "</div>";
        echo "</div>";
        ...
        if ($res_student == TRUE) {
            ...
            if ($count_student > 0) {
                while ($rows_student = mysqli_fetch_assoc($res_student)) {
                    ...
                    $sqlFetchResults = "SELECT * FROM tbl_results_sem6 WHERE enrollment_number = '$stu_id' AND Batch_id=$batch_id";
